...
- story: "אותה בקשה יצרה שתי תוצאות שונות."
- objective: "בחרו תוצאה שעומדת בקריטריון."

**הוראת הביצוע (מוצגת במסך ההרכבה):** "בדקו כל תוצאה מול הכלל."

**משוב על בחירה שגויה (שם את הרכיב החסר/עמום, לא חושף את התשובה):**
  - **"מדים כסופים ללא סמל"**: התוצאה נראית טובה, אבל חסר בה הסמל הנדרש. לא נבדקה מול קריטריון הצלחה — רק לפי מראה.

**הסבר הצלחה (מוצג עם "למה זה עבד?" לאחר ניצחון):**
"מדים כחולים עם סמל סוכנות עומדים בקריטריון שנקבע מראש — סמל ברור — ולכן הם הבחירה הנכונה, גם אם התוצאה השנייה נראתה סבירה. AI יכול להחזיר תוצאות שונות לאותה בקשה, ולכן בודקים כל תוצאה מול הקריטריון, לא לפי מראה."

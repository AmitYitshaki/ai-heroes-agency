...
- story: "תסבוכת חיברה את ההוראה והיומן לבלוק אחד."
- objective: "סכמו רק את יומן המסע."

**הוראת הביצוע (מוצגת במסך ההרכבה):** "בחרו סימון שמקיף את היומן."

**משוב על בחירה שגויה (שם את הרכיב החסר/עמום, לא חושף את התשובה):**
  - **"הוסיפו ״בבקשה״"**: זו מילת נימוס, לא גבול לנתונים. עדיין חסרה הפרדה ברורה בין ההוראה לנתונים.
  - **"סמנו רק ״תחילת היומן״"**: תחילת היומן ברורה, אבל הסוף עדיין פתוח. ההפרדה חייבת לסמן גם היכן הנתונים מסתיימים.

**הסבר הצלחה (מוצג עם "למה זה עבד?" לאחר ניצחון):**
"הסימון "תחילת היומן" ו"סוף היומן" מקיף בדיוק את הקטע לסיכום ומפריד אותו מההוראה עצמה. הפרדה בין הוראה לנתונים אומרת ל־AI היכן ההנחיה מסתיימת והחומר לעיבוד מתחיל."

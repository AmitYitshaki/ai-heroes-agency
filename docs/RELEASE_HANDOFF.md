...
**Release-candidate quality.** The full campaign is playable end to end: onboarding → character select → briefing → 23 battles across 4 regions + a tutorial + a finale → 4 workshop visits → 3 optional bonuses → certification ceremony. All 23 battles have been cross-checked against their authoritative specs (`handoff/source/04_BATTLE_CONTENT/battle_01.md`–`battle_23.md`) at least twice across different audit rounds; two content bugs were found and fixed (see §7). A dedicated pedagogical content pass (§7a, `docs/BATTLE_CONTENT_FINAL_AUDIT.md`) then sharpened every battle's wrong-answer feedback to name the specific missing/vague/conflicting prompt component and added a per-battle success explanation. Zero known P0s. One P1 remains, scoped to something this environment genuinely cannot verify (real screen reader, real device on-screen keyboard), not more engineering. Audio licensing for every shipped file is now project-owner-confirmed (see §6 and `docs/THIRD_PARTY_ASSETS.md`) — note the confirmed music license is personal/private/non-commercial use only, which matters if this build is ever deployed publicly.
...
src/
  ...
  components/   ui.tsx (Button, Modal, Stars, AppShell, briefcase panel, etc.), ErrorBoundary.tsx,
                ...
                EquippedLoop.tsx (renders Loop wrapped in whatever cosmetics are equipped)
  ...
  tests/        19 files, 133 tests — content, scoring, progress/persistence, finalBattle safety, battleFlow
                (all 23), audio, UI keyboard behavior, ErrorBoundary recovery, battle-option shuffle, bonus
                wheel geometry/anti-repeat, onboarding routing, villain pose mapping/fallback, battle_14
                reorder/recovery, equipped-cosmetics rendering and workshop integration
```
...
npm test             # vitest run — 19 files, 133 tests
...
14. **Equipped cosmetics render as small badges + an armor ring drawn entirely in CSS around Loop's existing artwork (`components/EquippedLoop.tsx`)** — no new character art. `COSMETIC_ICONS` in that file is the one manifest mapping each of the 12 catalog item ids to an existing `lucide-react` icon (already a dependency); position/color come from the item's own `slot`/`swatch`, not from the icon choice. Every `char_loop_*` render in the app goes through `EquippedLoop`, not raw `CharacterArt`, except the pre-journey landing-page splash (no progress exists yet to equip). Wrapping Loop's art broke three CSS rules that assumed `.character-art` was a *direct* child of `.outcome-panel`/`.victory-panel`/`.final-score` (`>` combinator) — they were relaxed to a descendant selector; don't re-introduce a `>` there without re-checking this wrapper.
15. **`BattleDefinition.successExplanation` (new field) is distinct from `concept`, and both are shown, at different moments, for a reason** — `concept` is the general principle, revealed *before* a choice via the "מה זה אומר?" progressive-disclosure `<details>` at compose; `successExplanation` is battle-specific ("why was *this* choice correct, which prompt component did it strengthen, how does that help an AI"), revealed *after* a win under "למה זה עבד?" in the outcome panel. Don't collapse them back into one field or reuse `concept` for both moments again — that was the exact staleness this round fixed (see `docs/BATTLE_CONTENT_FINAL_AUDIT.md`). `validateBattleRegistry` now rejects a battle with an empty `successExplanation`.
...
## 7a. Pedagogical content pass — instructions and learning feedback (03.09.2026)

A later round targeted quality, not correctness: the questions worked, but the task intro and post-answer explanations were judged "mediocre" — often not enough context to know what to do immediately, and success feedback that didn't explain *why* the answer was right or *which* prompt-engineering principle it reinforced. Full battle-by-battle audit matrix, before/after examples, and the exact wording changed are in **`docs/BATTLE_CONTENT_FINAL_AUDIT.md`** — read that file, not this summary, if you're touching battle content next.

In short: task intros (`story`+`objective`) were already tight and clear across all 23 battles (KEEP everywhere); every wrong-choice `outcome` that didn't already name its missing/vague/conflicting prompt component got a short clause added naming it, without revealing the answer; every battle gained a new `successExplanation` field (§5 item 15) rendered on success. Battle 23 is the one exception to "content lives in `battles.ts`" — its real copy lives in `FinalBattlePage.tsx`'s `outcomes` map, documented separately in the audit file and in `battle_23.md`'s synced addendum.

The pre-build production specs (`docs/handoff/source/04_BATTLE_CONTENT/battle_XX.md`) were **not** rewritten wholesale — they already diverged from shipped mechanics well before this round (a bespoke six-step help-ladder, `screenStateId` tables, asset IDs that were simplified away during build). Each file instead got a short appended "עדכון תוכן שוגר" section pointing at the code as the binding source and listing the final shipped strings, so the file no longer silently contradicts what ships without requiring a full spec rewrite.

...
import type { Battle23OutcomeKey, ComponentProvenance } from '../../schemas/game';
...
export function FinalBattlePage() {
  ...
  const finish = (provenance: ComponentProvenance) => {
    const value = calculateScore([provenance, provenance, provenance, provenance]);
  ...
  return <AppShell><article className="battle battle--finale">
    ...
    </div>{builderReady && <div className="assembled-prompt"><Check/><p>{approvedPrompt}</p></div>}<Button disabled={!builderReady} onClick={()=>finish('user_choice_two')}>{builderReady ? 'שגרו את התכנית' : 'השלימו את כל הרכיבים'}</Button></section>}
